...
public class IndexController extends BaseController {
	...
	public APIResponse saveProfile(@RequestParam String screenName, @RequestParam String email, HttpServletRequest request, HttpSession session) {
		// 如果进入这里即调用该post函数，session肯定是有User信息，但是如果别人直接跳到这个个人设置会怎么办？因为保存了cookie所以还是正常访问，
		// 如果我清理所有历史记录，cookie也自然清空了，再访问http://localhost:8888/admin/profile会怎么样？
		// 是否会显示admin/profile页面呢？ 事实上，经测试，直接显示admin/login页面，到底在哪里限制了一定要登录才能访问admin/profile？？？
		// 答案是拦截器，一般写在config的包路径，并配有@Configuration注解，或者以@Component的形式添加拦截器到WebMvcConfigurerAdapter
